...
const Storage = ({ straw, locknftId, onAdd }) => {
	...
	// delete unlockable
	 ...
	 const deleteUnlock = (lock) => {
		var delUnlock = [... straw];
		var due = delUnlock.findIndex(lock => lock.name == locknftId)
		if (due !== -1) {
	...
	<div>
		...
		<div>
			...
			<>
				...
				<br /><br /><button><a href={`https://filecoin-dlubh5ly6a-uc.a.run.app/owners/${lockable.name}`} target="_blank" title="API request for ownership" alt="API request for ownership">API to get owners</a></button><br />
